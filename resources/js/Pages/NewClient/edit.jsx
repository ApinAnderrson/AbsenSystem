        ...
        if (data.status === "Cicil" && jumlah > 0) {
          if (data.fase_pembayaran.length !== jumlah) {
            const fase = Array.from({ length: jumlah }, (_, i) => ({
              ...
              status_cicilan: data.fase_pembayaran[i]?.status_cicilan || 'false'
            }));
        ...
        put(route('new_client.update', clients.uuid), {
            onSuccess: () => {
                if(data.status === 'Cicil' || clients.cicilans.length !== 0){
                    router.delete(route('deleteCicilan.destroy', clients.uuid ), {
                        onSuccess: () => {
                                                                        ...
                                                                        <div key={index}>
                                                                            <div className='flex gap-2'>
                                                                                <input
                                                                                    ...
                                                                                    name={`fase-${index}`}
                                                                                    id={`fase-${index}`}
                                                                                    checked={fase.status_cicilan === 'true' || fase.status_cicilan === true}
                                                                                    onChange={(e) => {
                                                                                        const updatedCheck = [...data.fase_pembayaran];
                                                                                        updatedCheck[index] = {
                                                                                            ...updatedCheck[index],
                                                                                            status_cicilan: e.target.checked ? 'true' : 'false',
                                                                                        };
                                                                                        setData('fase_pembayaran', updatedCheck);
                                                                                    }}
                                                                                />
